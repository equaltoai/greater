<script lang="ts">
	...
	import type { Status, Relationship } from '$lib/types/mastodon';
...
const status = $derived.by(() => props.status);
const showThread = $derived.by(() => props.showThread ?? false);

// Cast to Lesser Status to access enhanced fields if available
const lesserStatus = $derived(status as unknown as LesserStatus);
...
const quoteMeta = $derived(displayStatus.quote_context || null);
	...
	onMount(async () => {
		if (!isOwnStatus && authStore.currentUser) {
			try {
				...
				if (!identifier) {
					console.warn('[StatusCard] Skipping relationship request: missing account id', {
	...
	async function handleFollow() {
		...
		try {
			...
			if (!identifier) {
				console.warn('[StatusCard] Cannot follow/unfollow: missing account id', {
					statusId: displayStatus.id,
					account: displayStatus.account,
				});
				return;
