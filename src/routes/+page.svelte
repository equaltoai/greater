<script lang="ts">
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          logError('[Home Timeline] Connection failed', { error });
  ...
  const actionHandlers: StatusActionHandlers = {
    onReply: (status) => logDebug('[Home Timeline] Reply action placeholder', status.id),
  ...
  <div class="timeline-page">
    ...
    {:else}
      <div class="error-state">
        <p>Please sign in to view your home timeline</p>
      </div>
    {/if}
  </div>
</TimelineLayout>
