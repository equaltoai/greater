<script lang="ts">
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          logError('[Profile Timeline] Connection failed', { error, username });
  ...
  const actionHandlers: StatusActionHandlers = {
    onReply: (status) => logDebug('[Profile Timeline] Reply action placeholder', status.id),
...
<TimelineLayout title={'Profile for ' + username} activeTab="profile">
  <div class="timeline-page">
    ...
    {:else}
      <div class="error-state">
        <p>Please sign in to view this profile timeline</p>
      </div>
    {/if}
  </div>
</TimelineLayout>
