<script lang="ts">
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          logError('[Local Timeline] Connection failed', { error });
  ...
  const actionHandlers: StatusActionHandlers = {
    onReply: (status) => logDebug('[Local Timeline] Reply action placeholder', status.id),
...
<TimelineLayout title="Local Timeline" activeTab="local">
  <div class="timeline-page">
    {#if currentUser}
      <div class="compose-section">
        <Compose.Root
          config={{ allowMedia: true, placeholder: "What's happening locally?" }}
    ...
    {:else}
      <div class="error-state">
        <p>Please sign in to view the local timeline</p>
      </div>
    {/if}
  </div>
</TimelineLayout>
